...
<template>
  ...
  <div class="form-base">
    ...
    <JHInput label="姓名" v-model="data.name" :valid="data.name != ''" notice="输入姓名"
      :type="pageStore.pageType == 'normal' ? 'normal' : 'mob'" />
    <JHInput label="电话号码" v-model="data.phone" :valid="isPhone(data.phone)" notice="请输入您的电话号码"
      :type="pageStore.pageType == 'normal' ? 'normal' : 'mob'" />
    <JHInput label="身份证号或学号" v-model="data.stu_id" :valid="isID(data.stu_id) || isStuId(data.stu_id)"
      notice="学长输入学号，新生输入身份证号" :type="pageStore.pageType == 'normal' ? 'normal' : 'mob'" />
    <JHInput label="E-mail" v-model="data.email" :valid="isEmail(data.email)" notice="输入Email"
      :type="pageStore.pageType == 'normal' ? 'normal' : 'mob'" />
    <JHInput label="验证码" v-model="data.code" :valid="data.code != ''" notice="输入邮箱验证码"
      :type="pageStore.pageType == 'normal' ? 'normal' : 'mob'" />
    <div class="btns">
