// 我们的部门
// 每年需要更新
// 当前数据：2026 年

type DepartmentDetail = {
  title: string;
  introduction: Array<string>;
  recruitment: Array<string>;
};

export const departmentContent: Record<string, DepartmentDetail> = {
  product: {
    title: "产品",
    introduction: [
      "产研部的“产”是产品的意思，我们既不敲代码也不制作实体产品，而是在精弘网络这样一个微型的互联网产品组织中扮演产品经理的角色，我们调研用户需求，思考功能架构，设计页面交互，致力于做出能够满足用户需求，符合用户期待，带来更多方便的互联网产品。不论是从学习、生活方面服务了广大学生的微精弘小程序，还是你现在所在浏览的精弘首页，亦或是即将到来的精弘论坛，都是我们的产品。我们的目标是用产品改变世界！"
    ],
    recruitment: [
      "产研部产品方向是包容万象的大家庭, 我们不限制专业, 不要求经验, 只需要你的热情、责任与一颗愿意学习的心,在这里,我们有部长团精心制作的课程, 为每一个对产品好奇的你提供学习的平台。如果你渴望参与一个互联网产品的设计，或是在校园里体验企业中产品经理这一职位,那产研部一定是你最好的选择！来到产品，你将收获到——欢快友爱的交流氛围，高效仔细的工作环境，志同道合的朋友伙伴,尽兴畅快的思维碰撞,或许还有命中注定的那个ta~心动不如行动,产品的大门向你敞开!"
    ]
  },

  technology: {
    title: "技术",
    introduction: [
      "我们是微精弘及各精弘应用平台的开发者和维护者。作为承载精弘团队几代人技术传承的小程序，微精弘是全体工大学子触手可及的校园生活管家，在学习、生活等多个场景中扮演着重要的角色。",
      "作为全校唯一的学生技术团队，我们始终保持着对技术的敬畏与热忱，每行代码背后都有同学们的思维碰撞与深入探索，每次迭代更新都见证着团队的协作成长与技术进步。在技术交流之外，我们更是一个温暖有爱的大家庭，在学习与生活中互帮互助、携手前行，一起面对困难，也一起分享收获，为了精益求精的共同目标不断努力。",
      "我们的主要开发技术栈: Go, HTML, CSS, TypeScript, JavaScript, Vue.js, Nuxt, Pinia",
      "如果你渴望用代码创造价值，期待在真实业务场景中提升能力，精弘产研部技术团队绝对是最适合你的选择！"
    ],
    recruitment: [
      "无论你是不是计算机专业的学生，无论你是否有过前后端开发的经验，只要你对技术感兴趣，想了解一个产品是如何从一个想法逐渐落地生根，欢迎加入精弘产研部技术团队的招新群。我们在这里一起写代码、做项目，也一起学习、成长。你会参与到真实的校园业务中，用技术为师生解决实际问题，也会在一次次版本迭代中体会项目推进的成就感。",
      "当你接触到前辈们留下的代码，可能会惊叹其中的巧思，也可能会被屎山折腾到红温。但它们之所以还能被使用，是因为它们承载的不仅仅是功能，更是一代代精弘人的经验与努力，是一代代技术人留下的印记。",
      "而你，愿不愿意也在这份印记中，留下自己的注脚？"
    ]
  },

  affairs: {
    title: "常务处",
    introduction: [
      "Hello大家好！这里是精弘网络常务处——组织的核心纽带，团队高效运转的强力保障！",
      "我们活跃在学校的三个校区，始终是精弘网络最坚实的后盾。",
      "我们的工作主要分为两大板块：",
      "日常运营：会议记录与跟进、跨部门协调与资源整合、重要文书起草与管理",
      "活动支持：从创意策划到落地执行，从物资筹备到后勤保障，我们全程护航每一场精彩活动！",
      "同时，我们还肩负着外联沟通的重要使命。既要脑洞大开设计趣味方案，又要化身谈判达人争取优质资源。在这里，你不仅能收获全方位的技能成长，更能遇见一群温暖又有趣的灵魂！",
      "工作认真，玩得更嗨！我们的团建清单永远充满惊喜：深夜海底捞、凌晨追日出、通宵KTV......在常务处，我们誓要做既能扛事又会搞事的超级团队！"
    ],
    recruitment: [
      "亲爱的学弟学妹们～欢迎加入常务处的温暖小窝！我们不仅是高效运转的团队，更是彼此陪伴的家人！",
      "在这里，例会从不枯燥——它是每周一次的欢乐聚会！",
      "在这里，你的生日绝不会被遗忘——全员的祝福，仪式感拉满！",
      "在这里，学长学姐24小时在线——学习、生活、情感……任何问题，我们随时“捞”你！",
      "常务处，不只是一个部门——",
      "它是你大学生活的“第一站温暖”，是随时可以回来的“家”！",
      "让我们一起期待在常务处的每一个明天！！！"
    ]
  },

  design: {
    title: "小弘工作室",
    introduction: [
      "小弘工作室是精弘网络的一个创新创意服务部门，针对学校活动的宣传方案进行设计。部门成员之间会进行协作，为活动的宣传制作一套完整的设计方案。",
      "主要活动:精弘毅行、网络安全宣传月、网络文化节等。",
      "主要事物:海报设计、喷绘设计、UI界面设计、精弘网络周边文创类产品设计以及各类视觉方案设计。",
      "在小弘工作室，你不仅能够交到兴趣相似的伙伴，还能够丰富课余生活，提升专业技能。除了基本的聚餐、轰趴、给小部员过生日等福利以外，部门还开设了ps、ai等设计软件的课程以及设计方面的各种技巧教学，我们注重对部门成员综合设计技能的培养与提升。",
      "在大学里加入很多部门之后，你可能会发现繁冗的零碎事情太多，占用了太多的课余时间，但小弘工作室不会。你一定能得到你想要的进步。全方位包装，pick的就是你啦~"
    ],
    recruitment: [
      "如果你对设计怀着一份好奇向往与憧憬，那么小弘工作室一定是你入门设计的好地方。在这里，你不仅能结识有相同兴趣爱好的小伙伴，还能习得一些设计知识及掌握部分设计技能。只要你怀有一颗喜爱设计的心，我们欢迎你的加入~"
    ]
  },

  visual: {
    title: "视觉影像部",
    introduction: [
      "视觉影像隶属新媒体，是校园学习生活、学校各类大型活动的记录团队，(我们)专注于摄影与视频前期拍摄和后期剪辑，致力于用最优秀的制作，最独特的创意呈现最美的工大人、工大景，传播技术知识。视觉影像期待用最用心的摄制给你带来最高级的视觉盛宴！欢迎关注b站:485566103。（隶属于新媒体的视觉影像有着脱单魔法，想要脱单并喜欢拍摄与后期制作的小朋友们，走过路过不要错过哦）"
    ],
    recruitment: [
      "我们有最纯粹的激情和最单纯的热爱，致力于用照片封存每个美好瞬间，用视频记录每段美好故事。如果你擅长用构图诉说情绪，用剪辑掌控节奏，或用色彩渲染氛围——无论你是镜头后的观察者，还是屏幕前的魔法师，这里都有一块属于你的创作原野。加入我们，一起把转瞬即逝，变成历久弥新。"
    ]
  },

  editorial: {
    title: "编辑工作室",
    introduction: [
      "编辑工作室是精弘网络的“喉舌”部门，负责内容创作和相关运营工作。“浙工大精小弘在线”、“浙江工业大学精弘网络”微信公众号是我们的工作重心。",
      "我们的工作有极强的综合性：文稿撰写需要语言文学知识，创作和阅读积累对工作也有很大帮助；推送排版需要平面设计知识，HTML、SVG等数字技术知识同样用得上；运营分析、读者心理、互动技巧、文化建设等社科类技能，更是关键的助推器。我们常常需要与各部门的伙伴沟通协作，完成各类产品、服务、活动的宣传工作。",
      "选题选材，撰文撰稿，图文排版，读者互动；奇思妙想、蛛丝马迹是我们的素材，咬文嚼字、精雕细琢是我们的技艺。手执工具，掌握信息，眼观风向，脚踏平台，肩负使命，心承热爱，让文字开花结果，让想法生根发芽，用创意和本领浇铸网络文化，用艺术和技术描摹校园天地，用图文和视效叙写工大故事。"
    ],
    recruitment: [
      "如果你对公众号运营有好奇、如果你对推送时常关注、如果你脑海中有各色各样的灵感想找一个平台来实现、如果你略懂排版抑或是擅长文字撰写，编辑工作室绝对是你的不二之选。",
      "写推文、做策划、玩排版，既能静心码字，也能欢乐团建，是新媒体入门的绝佳选择。我们让文字绽放光彩，让创意落地生长，用审美描摹校园天地，用图文叙写工大故事。",
      "期待与你在编辑工作室进行思想碰撞、同频共鸣！"
    ]
  }
};

export const technologyImages: string[] = [
  "groupPhoto/technology0.webp",
  "groupPhoto/technology1.webp",
  "groupPhoto/technology2.webp",
  "groupPhoto/technology3.webp",
  "groupPhoto/technology4.webp",
  "groupPhoto/technology5.webp"
];

export const affairsImages: string[] = [
  "groupPhoto/2026/changwu0.webp",
  "groupPhoto/2026/changwu1.webp",
  "groupPhoto/2026/changwu2.webp",
  "groupPhoto/2026/changwu3.webp"
];

export const designImages: string[] = [
  "groupPhoto/2026/xiaohong0.webp",
  "groupPhoto/2026/xiaohong1.webp",
  "groupPhoto/2026/xiaohong2.webp"
];

export const visualImages: string[] = [
  "groupPhoto/visual0.webp",
  "groupPhoto/visual1.webp",
  "groupPhoto/visual2.webp"
];

export const editorialImages: string[] = [
  "groupPhoto/editorial0.webp",
  "groupPhoto/editorial1.webp",
  "groupPhoto/editorial2.webp"
];
